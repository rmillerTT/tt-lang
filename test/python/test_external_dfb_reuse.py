...
def test_conditional_external_dfb_reuse_across_reconfiguration(
    ...
    # every lifetime is proven complete before the state-discarding
    # reconfiguration that follows it and all three DFBs share one index.
    assert _get_final_physical_dfb_indices(final_mlir_path) == [0]
    actual = ttnn.to_torch(result).float()
    if dtype == torch.bfloat16:
